...
def make_withdraw(balance, password):
    ...
    def withdraw(amount,word):
        nonlocal password,balance,lis,num
        if word == password and num <3:
            if amount <= balance:
                balance -= amount
                return balance
            else:
            ...
            lis += [word]
            if num > 3:
                return "Your account is locked. Attempts:" + str(lis[:3])
            else:
...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.banlance < self.price:
            return 'You must deposit ${0} more.'.format(self.price - self.banlance)
        self.stock -= 1
        change = self.banlance - self.price
        self.banlance = 0
        if change == 0:
    ...
    def deposit(self,amout):
        if self.stock <= 0:
            return 'Machine is out of stock. Here is your ${0}.'.format(amout)
        self.banlance += amout
        return 'Current balance: ${0}.'.format(self.banlance)
      ...
      2
        1
          2
            ...
            5
          5
    >>> peath_to_2 = generate_paths(t2, 2)
